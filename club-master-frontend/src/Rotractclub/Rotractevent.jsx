...
import { Heart, MessageCircle, Share2, Calendar, MapPin, Clock, Trash2, Edit, X, Check } from 'lucide-react';
...
const RotracEvent = () => {
    ...
    try {
      ...
      const eventsWithComments = await Promise.all(
        response.data.map(async (event) => {
          const commentsResponse = await axios.get(`http://localhost:7000/api/v1/comment/allComments/${event.eventId}`);
          return {
            ...event,
  ...
  const handleAddComment = async (eventId) => {
    ...
    try {
      ...
      console.log('Error adding comment', error.response ? error.response.data : error.message);
    }
  };
  ...
  const formatDateTime = (dateArray, timeArray) => {
    ...
    const date = new Date(dateArray[0], dateArray[1] - 1, dateArray[2], timeArray[0], timeArray[1], timeArray[2] || 0);
      ...
      <div className="min-h-screen bg-gradient-to-tl from-indigo-900 via-purple-900 to-pink-900">
        <div className="max-w-6xl mx-auto p-8 pt-32">
          ...
          <div className="grid grid-cols-1 gap-12">
            {events.length > 0 ? (
              events.map(event => (
                <div key={event.eventId} className="relative bg-black/30 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/10">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-6">
                        ...
                        <div className="w-full aspect-video bg-gray-700/50 rounded-xl flex items-center justify-center text-cyan-100 overflow-hidden">
                          <img src='/bloodeventroc.jpg' alt={event.eventName} className="w-full h-full object-cover" />
                        </div>
                        <div className="flex flex-wrap gap-4">
                          <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-2 rounded-full text-cyan-100">
                            <Calendar className="w-4 h-4" />
                            <span>{formatDateTime(event.scheduledDate, event.scheduledTime)}</span>
                          </div>
                          <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-2 rounded-full text-cyan-100">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-4 max-h-64 overflow-y-auto pr-4">
                            {event.comments && event.comments.map((comment) => (
                              <div key={comment.commentId} className="flex items-start gap-3 bg-white/5 backdrop-blur p-4 rounded-xl">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-400 to-blue-400 flex items-center justify-center text-white font-medium">
                                  {comment.memberName ? comment.memberName[0] : "U"}
                                </div>
                                
                                <div className="flex-1">
                                  <div className="flex justify-between items-start">
                                    <div className="text-cyan-300 text-sm mb-1">
                                      {comment.memberName || "User"} • {formatCommentDate(comment.dateTime)}
                                    </div>
                                      ...
                                      <div className="flex gap-2">
                                        {editingComment !== comment.commentId ? (
                                          <>
                                            ...
                                            <button 
                                              onClick={() => handleDeleteComment(event.eventId, comment.commentId)}
                                  ...
                                  {editingComment === comment.commentId ? (
                                    ...
                                    <p className="text-cyan-100">{comment.comment}</p>
                                  )}
                                </div>
                              </div>
                            ))}
                  ...
                  <div className="relative border-t border-white/10 p-4 bg-white/5 backdrop-blur">
                    <div className="flex justify-between w-full">
                      ...
                      >
                        ...
                        <span>{event.comments ? event.comments.length : 0}</span>
                      </button>
